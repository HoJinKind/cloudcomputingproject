...
def lambda_handler(event, context):
    ...
    id = event['id']
    user = event['user']
    table = dynamodb.Table('cc_project_access')      
    resp = table.get_item(
            Key={
                'id' : id
            }
    ...
    print(resp)
    resp['Item']['access'].add(user)
    deviceAccess = {
        'id': "1",
        'access': resp['Item']['access']
    }
    print(deviceAccess)
    table.put_item(Item=deviceAccess)
    final_text = "The device " +id + " has been confirmed! You are now listening for any falls detected by the device."
    url = URL + "sendMessage?text={}&chat_id={}".format(final_text, user)
    requests.post(url)
